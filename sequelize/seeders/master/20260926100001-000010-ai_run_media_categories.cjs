 ...
 *
 * Three rows — image, video and audio. Only image is handled this version, and the other two are
 ...
 *
 * `is_active` carries which of the three this version handles, so turning video on later is a flag
 * flipped on an existing row and a fourth kind is a new row. Neither is a change to a column.
 *
...
const seeds = [
  {
    ...
    is_active: AI_RUN_MEDIA_CATEGORY.IMAGE.IS_ACTIVE,
  },
  {
    ...
    is_active: AI_RUN_MEDIA_CATEGORY.VIDEO.IS_ACTIVE,
  },
  {
    ...
    is_active: AI_RUN_MEDIA_CATEGORY.AUDIO.IS_ACTIVE,
  },
]
